...
def create_ropes(env_dict):
    ...
    for body in env_dict["mujoco"]["worldbody"]["body"]:
    ...
    def create_pole_ropes(pole_name, rotated_prefix, quat_rotation, identity_rotation):
        nonlocal composite_list
    ...
    create_pole_ropes("pole_bottom_left", "tl", "0.7071 0 0 0.7071", "1 0 0 0")
    # Cable goes in opposite direction
    create_pole_ropes("pole_top_right", "br", "0.7071 0 0 -0.7071", "0 0 1 0")

    # Add the data back to the xml
    env_dict["mujoco"]["worldbody"]["composite"] = composite_list
    env_dict["mujoco"]["equality"] = {"connect": connection_list}
    return env_dict